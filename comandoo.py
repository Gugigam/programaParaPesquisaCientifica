...
def adciona_linha_em_csv(dados_gerais_int, exe, nome_do_problema, pasta):
	with open(f'{pasta}/{nome_do_problema}_mem.csv', 'a+') as stream:
...
def adciona_linha_em_csv_media(dados_gerais_media_int, exe, nome_do_problema, pasta):
	with open(f'{pasta}/{nome_do_problema}_mem_media.csv', 'a+') as stream:
...
pasta = sys.argv[1]
vetorQualquer = sys.argv[1].split("/")
vetorQualquer2 = vetorQualquer[-2].split("_")
nome_do_problema = vetorQualquer2[0]
...
testes = os.listdir(f"{pasta}/test")
arqtxt = []
for test in testes:
	print(test)
...
def fastlangslow(pasta, typeArq = "typeArq"):
	arquivos = os.listdir(f"{pasta}/{typeArq}")
	arqexe = []
	...
	for exe in arqexe:
		...
		for test in testes:
			comando = f"valgrind ./{pasta}{typeArq}/{exe} < {pasta}test/{test} > /dev/null 2>arquivoo.txt"
			print("Vou executar " + comando) 
			os.system(comando)
			arquivoo = "arquivoo.txt"
			...
			adciona_linha_em_csv(dados_gerais_int, exe, nome_do_problema, pasta)
			for i in range(0,8):
		...
		adciona_linha_em_csv_media(dados_gerais_media_int, exe, nome_do_problema, pasta)
...
fastlangslow(pasta,typeArq = "fast")
...
#sumarryss = {"exit" : dados_gerais_int[0],
#			"usage" : dados_gerais_int[1],
#			"free" : dados_gerais_int[2],
#			"allocated" : dados_gerais_int[3],
#			"definitely" : dados_gerais_int[4],
#			"indirectly" : dados_gerais_int[5],
#			"possibly" : dados_gerais_int[6],
#			"reachable" : dados_gerais_int[7]
#}
